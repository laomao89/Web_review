      ...
      <table>
        ...
        <tfoot>
          <tr>
            <td colspan="5">
              <span>总分：{{zongf}}</span>
              <span style="margin-left:50px">平均分：{{junf}} | {{(zongf/list.length).toFixed(2)}} </span>
            </td>
          </tr>
        </tfoot>
...
export default {
  ...
  computed:{
    zongf(){
      // 分数求和  reudce复习
      let restult = this.list.reduce((sum,item)=>sum+=item.score,0).toFixed(2);
      return restult
    },
    junf(){
      let res = (this.zongf/this.list.length).toFixed(2)
      return res;
    },
